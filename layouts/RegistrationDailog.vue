      ...
      <v-card class="pa-6">
        ...
        <form @submit.prevent="createUserAccount()">
          ...
          <div class="d-flex justify-center">
            <div class="page-link" @click="showLoginModal">
              Already have an account? Log in
            </div>
          </div>
        </form>
      </v-card>
      <set-user-token v-if="signUp" :user-data="userData" />
    </v-dialog>
  </v-row>
</template>
...
export default class Login extends Vue {
  ...
  async createUserAccount(): Promise<void> {
    ...
    try {
      ...
      const registration: AuthenticateUserResponse = await this.$api.register(
        userData
      )
