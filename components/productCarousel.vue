<template>
  ...
  <div v-else class="no-product-notice">
    No product was found in <span> {{ categoryName }} </span> category
  </div>
</template>
<script lang="ts">
...
export default class ProductSlider extends Vue {
  ...
  @Watch('productListing', { immediate: true, deep: true }) checkRecipient(
    newVal: SliderProduct
  ) {
